...
当前仓库已完成本方案规划的 React-only + Vite-only + weapp/Skyline-first 现代化改造，`1.2.0` 稳定线已落地。
...
- `@spcsn/taro-cli` 已移除对 `@spcsn/taro-components` 与 `@spcsn/taro-shared` 的直接依赖，平台能力改由 `@spcsn/taro/runtime` 子路径承接。
...
```text
packages/
  taro/                        # Taro API 门面 + 小程序运行时
    src/runtime/               # 原 @spcsn/taro-runtime 源码内联目录，构建后通过 @spcsn/taro/runtime 子路径暴露
...
### 4.4 `@spcsn/taro-runtime`（已内联至 `packages/taro/src/runtime/`）

当前状态：原 `@spcsn/taro-runtime` 包已源码级内联到 `packages/taro/src/runtime/`，由 `@spcsn/taro` 统一构建，并通过 `@spcsn/taro/runtime` 子路径暴露，供 CLI 构建期引用。该子路径不面向业务应用，业务仍只依赖 `@spcsn/taro`。
...
当前状态：`@spcsn/taro-plugin-platform-weapp` 已并入 `@spcsn/taro-cli`，不再作为公开插件包。相关源码保留在 CLI 内部实现（`packages/taro-cli/src/platform-weapp/` 及 `packages/taro-cli/src/internal/` 相关模块），随 `@spcsn/taro-cli` 一起构建发布。
...
当前 `@spcsn/taro-cli` 仅支持以下命令：

- `taro init`
- `taro build`（默认 `--type weapp`，`--watch` 用于开发模式）
...
当前状态：独立 `babel-preset-taro` 包已移除，相关 Babel 转换能力已内联到 `@spcsn/taro-cli` 内部 transform 管线（源码位于 `packages/taro-cli/src/internal/` 及相关模块），不再作为业务或公开包。
...
```json
{
  "dependencies": {
    "@spcsn/taro": "1.2.0",
    "@spcsn/taro-components": "1.2.0",
    "react": "^19.2.0"
  },
  "devDependencies": {
    "@spcsn/taro-cli": "1.2.0"
  }
}
```

业务工程不应显式安装 `@spcsn/taro-react`、`@spcsn/taro-plugin-platform-weapp`、`@spcsn/taro-plugin-framework-react`、`@spcsn/taro-mini-runner`、`@spcsn/taro-runtime`、`@spcsn/taro-shared` 或 `@spcsn/taro-helper`。这些内部能力已源码级内联到 `@spcsn/taro` 或 `@spcsn/taro-cli` 中，不再作为独立 npm 包存在或传递安装。
...
- `@spcsn/taro-runtime`、`@spcsn/taro-shared`、`@spcsn/taro-helper`、`@spcsn/taro-service`、`@spcsn/taro-mini-runner` 已不再作为独立 npm 包存在，相应能力已内联到 `@spcsn/taro` 或 `@spcsn/taro-cli` 中，不应由业务显式声明。
...
不要为了修一个构建问题把以下已归档或已内联的内部实现重新写进业务 `package.json`：
