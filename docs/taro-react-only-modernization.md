...
- `@spcsn/taro-runtime -> @spcsn/taro-shared` 仍是运行时硬依赖：剩余阻塞集中在 `internalComponents/getComponentsAlias`、`processApis/merge*`、`UnRecursiveTemplate` 三类能力，暂不满足移除 direct dependency 的条件。
