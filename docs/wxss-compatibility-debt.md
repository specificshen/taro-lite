...
- `packages/taro-cli/src/internal/runner/plugins/vite-plugin-wxss-compat.ts`
- `packages/taro-cli/src/internal/runner/style-transforms/wxss-compat.ts`

### 2. LightningCSS targets 配置

`packages/taro-cli/src/internal/runner/mini-program/config.ts` 中将 `build.target` 保持为 `es2022`（满足 JS 现代化输出），同时单独为 CSS 配置偏旧的浏览器 targets：
