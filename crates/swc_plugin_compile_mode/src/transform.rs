...
impl TransformVisitor {
                      ...
                      match &mut **expr {
                        ...
                        Expr::Call(CallExpr {
                          ...
                          if callee_expr.is_member() {
                            let expr_string = as_xscript_expr_string(
                            ...
                            if expr_string.is_some() {
                              ...
                              let args_string: Vec<String> = args
                                ...
                                .map(|arg| {
                                  match &*arg.expr {
                                    ...
                                    _ => {
                                      ...
                                      attrs_wait_for_inserting.push(JSXAttrOrSpread::JSXAttr(
                                        JSXAttr {
                                          span,
                                ...
                                .collect();
                              // 将结果输出到 <template>
                              let miniapp_attr_value = utils::gen_template(&format!(
                                "{}({})",
                                expr_string.unwrap(),
                                args_string.join(",")
                              ));
  ...
  fn build_xml_children(
    ...
    if jsx_exprs_wait_for_inserting.len() > 0 {
      let mut xs_args_ids: Vec<u32> = jsx_exprs_wait_for_inserting
        .keys()
        .map(|k| k.clone())
        .collect();
      xs_args_ids.sort();
...
impl VisitMut for TransformVisitor {
  ...
  fn visit_mut_module_items(&mut self, body_stmts: &mut Vec<ModuleItem>) {
    ...
    keys.sort();
    let mut stmts_being_inserted: Vec<ModuleItem> = keys
      .into_iter()
      .map(|key| {
    ...
    if self.xs_sources.len() > 0 {
      let elems: Vec<Option<ExprOrSpread>> = self
        ...
        .map(|item| {
          Some(ExprOrSpread {
            spread: None,
            expr: Box::new(Expr::Lit(Lit::Str(quote_str!(item.clone())))),
          })
        })
        .collect();
